...
#function for lengthening demographic info and tidy map plots.

#pre: data is defined tibble containing a community in Name and data in value
#pre: community is defined and contains an SF geometry and community name in community
#pre: demo_col is user_defined text string and is contained in data
#post: outputs a ggplot map of region with fill based on data value.  


demo_plot<-function(data, demo_col, mainTitle=""){
     data%>%
          dplyr::select(Name, starts_with(demo_col))%>%
          pivot_longer( 
               cols= starts_with(demo_col), 
               names_to= "demo_year")%>%
          inner_join(communities, by=c("Name"="community"))%>%
          st_as_sf()%>%
          ggplot()+
          geom_sf(aes(fill=value, geometry=geometry))+
          #facet_wrap(vars(demo_year))+
          labs(title=mainTitle)+
          scale_fill_viridis_c(option="C")
}

png("graphics/demo_pop.png", width=6.5, height=9, units="in", res=144)
demo_plot(demo.population,"POP_2014", "Community Population, 2014-2018")
png("graphics/PC_Income.png", width=6.5, height=9, units="in", res=144)
demo_plot(demo.income, "PCI_2014", "Per Capita Income, 2014-2018")
dev.off()
...
png("graphics/viol_pct_as_key.png", width=6.5, height=9, units="in", res=144)
vio_pct_as_key.plot
dev.off()
```
...
png("graphics/viol_pct_prsnt.png", width=9, height=6.5, units="in", res=144)
viol_pct_prsnt.plot
dev.off()
```
...
png("graphics/key_viol_pct.png", width=6.5, height=9, units="in", res=144)
key_viol_pct.plot
dev.off()
```
...
png("graphics/key_viol_pct_prsnt.png", width=9, height=6.5, units="in", res=144)
key_viol_pct_prsnt.plot
dev.off()
```
...
png("graphics/key_viol_pct_w_class.png", width=6.5, height=9, units="in", res=144)
key_viol_pct_w_class.plot
dev.off()
```
...
```{r}
png("graphics/key_viol_pct_w_class_prsnt.png", width=18, height=6, units="in", res=144)
violations%>%
     dplyr::select(V_Num, `Inspection ID`)%>%
     inner_join(inspect_long)%>%
     dplyr::filter(Results %in% c("Pass", "Pass w/ Conditions", "Fail"))%>%
...
#Graphs...
png("graphics/biz_ct_map.png", width=6.5, height=9, units="in", res=144)
#show businesses per community.
cmty_stat_plot(biz_by_community,"biz.ct", "Businesses per Community", "Count")
dev.off()

png("graphics/biz_year_map.png", width=6.5, height=9, units="in", res=144)  
...
dev.off()
png("graphics/biz_insp_per_yr.png", width=6.5, height=9, units="in", res=144)  
...
#this is interesting.  Look at the higher number of violations in outer areas.
png("graphics/biz_viol_vs_cmty.png", width=6.5, height=9, units="in", res=144)
...
png("graphics/insp_fail_vs_cmty.png", width=6.5, height=9, units="in", res=144)
...
png("graphics/biz_fail_map.png", width=6.5, height=9, units="in", res=144)
